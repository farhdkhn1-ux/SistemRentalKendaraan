                ...
                <form action="{{ route('admin.rentals.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="col-span-2">
                            <label class="block text-sm text-gray-700 dark:text-gray-300 mb-1">Pilih Kendaraan</label>
                            <select name="vehicle_id" class="w-full border rounded px-3 py-2 dark:bg-gray-700 dark:text-white" required>
                                <option value="">-- Pilih Kendaraan --</option>
                                @foreach($vehicles as $v)
                                    <option value="{{ $v->id }}" {{ old('vehicle_id') == $v->id ? 'selected' : '' }}>
                                        {{ $v->brand }} {{ $v->model }} ({{ $v->plate_number }}) - Rp {{ number_format($v->daily_rate, 0, ',', '.') }}/hari
                                    </option>
                                @endforeach
                            </select>
                            @if($vehicles->isEmpty())
                                <p class="text-red-500 text-sm mt-1">Tidak ada kendaraan yang tersedia saat ini.</p>
                            @endif
                        </div>
                        ...
                        <div class="col-span-2">
                            ...
                            <textarea name="notes" rows="3"
                                class="w-full border rounded px-3 py-2 dark:bg-gray-700 dark:text-white">{{ old('notes') }}</textarea>
                        </div>
